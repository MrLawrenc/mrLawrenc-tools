...
public class CompileUtil {
     ...
     * @see com.github.mrlawrenc.utils.clz.CompileUtil#compileFileOutMemory(Map, File)
     * @see com.github.mrlawrenc.utils.clz.CompileUtil#compileSourceCode2Memory(String, String)
     */
     ...
     */
    public static ForwardingJavaFileManager<StandardJavaFileManager> out2MemoryJavaFileManager(StandardJavaFileManager fileManager, Map<String, byte[]> bytes) {
    ...
    public static Map<String, byte[]> compileSourceCode2Memory(String className, String sourceCode) throws Exception {
        //bytes      编辑之后的class数据存储在该map中，key为全限定名，value为class字节数组
        Map<String, byte[]> bytes = new HashMap<>(2);
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        StandardJavaFileManager fileManager = compiler.getStandardFileManager(null, null, null);
        ForwardingJavaFileManager<StandardJavaFileManager> jfm = out2MemoryJavaFileManager(fileManager, bytes);
